...
9. visual.contains_product는 판매/추천 대상 제품이나 그 제품의 명확한 이미지가 화면에 보일 때 true다. 쇼핑몰 상품페이지나 상품사진에 대상 제품이 보이는 경우도 true다. 단순 관련 배경이나 범용 소품만 보이면 false다.
10. action.type은 도포, 타건, 조리, 시식, 착용, 분사, 세척, 측정, 제품제시처럼 화면에서 실제 일어난 행동을 간결하게 적는다. 행동이 없으면 "정적제시"처럼 적는다.
11. message_roles는 훅, 제품소개, 문제제기, 사용시연, 기능설명, 결과제시, 비교, 큐레이팅, 후기, 가격/혜택, CTA 등 해당 구간이 영상에서 수행하는 역할을 복수로 적을 수 있다.
12. claims에는 발화나 자막이 사실이라고 주장하는 내용을 원문의 의미를 바꾸지 말고 기록한다. 주장 자체가 없으면 빈 배열로 둔다.
13. evidence는 주장과 분리한다.
...
14. 제품을 먹는 장면, 바르는 장면, 착용하는 장면 자체를 효능/성능의 "관찰가능한결과"로 처리하지 않는다.
15. 잠든 연출, 행복해진 표정 같은 재연을 실제 효과가 확인된 결과로 처리하지 않는다.
16. evidence.observable_result에는 화면에서 결과로 직접 관찰 가능한 변화만 적는다. 없으면 빈 문자열이다.
17. evidence.result_visually_observable은 실제 결과 변화가 화면에서 확인될 때만 true다.
18. evidence.types는 항상 하나 이상 기록한다. 대응 근거가 없다면 "근거없음"을 사용한다.
19. 소재 출처나 빠른 화면에 확신이 없으면 confidence를 medium/low로 낮추고 추측 대신 불명확을 사용한다.
`;
