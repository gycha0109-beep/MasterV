...
function buildProductionSteps(analysis: VideoAnalysis) {
  ...
  if (steps.length === 0) {
    steps.push({ title: "전체 구성", detail: analysis.structure_label });
  }
  return steps;
}
...
function productInfoItems(analysis: VideoAnalysis) {
  const claims = unique(analysis.observation_segments.flatMap((segment) => segment.claims).map(clean).filter(Boolean));
  if (claims.length === 0) return ["제품명·가격·핵심 특징 등 실제 상품 정보 확인"];
  return claims.slice(0, 5).map((claim) => `확인: ${shorten(claim, 62)}`);
}
...
function buildAssetGroups(analysis: VideoAnalysis): AssetGroup[] {
  ...
  const info = productInfoItems(analysis);
...
function baseContext(
  ...
  guide: Pick<SingleVideoProductionGuide, "direction_summary" | "production_steps" | "asset_groups" | "critical_warnings">
) {
