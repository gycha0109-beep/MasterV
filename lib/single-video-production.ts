...
function stepTitle(segment: ObservationSegment, index: number) {
  const source = `${segment.scene_purpose} ${segment.action.type} ${segment.message_roles.join(" ")}`;
  const isComparisonExample =
  ...
  if (isComparisonExample) {
    if (index === 0 && segment.message_roles.includes("문제제기")) return "문제 사례";
    return "비교 사례";
  }
...
function buildProductionSteps(analysis: VideoAnalysis) {
  ...
  analysis.observation_segments.forEach((segment, index) => {
    const next = { title: stepTitle(segment, index), detail: stepDetail(segment) };
    const previous = allSteps.at(-1);
    if (previous && previous.title === next.title && previous.detail === next.detail) return;
    allSteps.push(next);
  });
...
function buildAssetGroups(analysis: VideoAnalysis): AssetGroup[] {
  ...
  return [
    ...
    },
    {
      title: "상품 정보",
      icon: "📝",
      items: ["아래 '내 상품 정보'에 실제 판매 상품의 확인된 사실 입력"]
    }
  ];
}
...
function baseContext(
  ...
  guide: Pick<SingleVideoProductionGuide, "direction_summary" | "production_steps" | "asset_groups" | "reference_claims" | "critical_warnings" | "product_truth">
) {
...
export function compileSingleVideoProductionGuide(
  ...
  const prompts: Record<SingleVideoPromptKind, string> = {
    script: `${base}\n\n[작업: 대본]\n이 제작 구조를 내 상품 정보에 맞춰 새롭게 적용한 세로형 상품 숏폼 대본을 작성한다. 각 구간마다 시간, 화면, 행동, 내레이션/대사, 화면 자막, 장면 목적을 작성한다. 첫 3초와 CTA를 명확히 구분하고, 내 상품 정보에 없는 사실은 [확인 필요]로 표시한다.`,
    shooting: `${base}\n\n[작업: 촬영]\n내 상품으로 실제 촬영 가능한 쇼트 리스트를 작성한다. 각 쇼트마다 예상 길이, 피사체, 행동, 구도/카메라, 필요한 소품, 화면에서 반드시 확인되어야 할 내용을 적는다. 비교 사례·연출 B-roll과 내 상품 직접 시연을 명확히 구분하고, 확인되지 않은 참고영상 주장을 촬영 전제로 사용하지 않는다.`,
    ...
    editing: `${base}\n\n[작업: 편집]\n편집자용 지시서를 작성한다. 시간순 컷 구성, 첫 3초 처리, 컷 전환 목적, 자막 밀도, 제품 노출 타이밍, 시연/결과 장면 유지 시간, CTA 처리, 과장 표현 금지 항목을 포함한다. 비교·일반 사례나 B-roll을 내 상품 성능 입증처럼 편집하지 않는다.`
  };
