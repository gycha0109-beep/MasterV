...
assert(appText.includes("backend.remoteOperations.discoverYouTube(session, query, options)"), "desktop discovery must delegate through RemoteOperationClient");
assert(!appText.includes("masterv-api-boundary"), "desktop discovery consumer must not own hosted endpoint");
assert(!appText.includes('operation: "youtube_discovery"'), "desktop discovery consumer must not own hosted operation payload");
...
assert(hostedStart >= 0 && hostedEnd > hostedStart, "legacy hosted discovery adapter missing");
assert(hostedBlock.includes('requestBoundary(session, "POST"'), "desktop discovery adapter request must use POST");
assert(hostedBlock.includes('operation: "youtube_discovery", query, options'), "desktop discovery adapter must send operation/query/options only");
...
console.log(JSON.stringify({ status: "MASTERV_HOSTED_YOUTUBE_DISCOVERY_CONTRACT_PASS", canonical_source_commit: SOURCE_COMMIT, canonical_blob_count: Object.keys(expectedBlobs).length, provider_authority: "hosted-secret", desktop_provider_credentials: false, desktop_transport_owner: "remote-operation-provider", analysis_authority: "metadata-only", local_next_api_required: false, gemini_dependency: false }));
