...
const runtimeFiles = walk(desktopRoot)
  .filter((file) => /\.(?:js|html)$/i.test(file))
  .map((file) => ({ file, relative: rel(file), source: fs.readFileSync(file, "utf8") }));
...
const forbiddenGenericTokens = [
  ...
  "/rest/v1",
  "/functions/v1",
  "MASTERV_SESSION_BRIDGE",
  "getAccessToken",
  "session-bridge.js"
];

for (const { relative, source } of genericRuntime) {
  for (const token of forbiddenGenericTokens) {
    assert.equal(source.includes(token), false, `${relative} contains forbidden generic runtime coupling: ${token}`);
  }
...
for (const token of ["NEXT_PUBLIC_SUPABASE_URL", "NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY", "NEXT_PUBLIC_MASTERV_API_BASE_URL", "supabase_url", "supabase_publishable_key", "api_base_url"]) {
  assert.equal(bridge.includes(token), true, `legacy config bridge lost transitional mapping: ${token}`);
}
...
for (const token of ["supabase_url", "supabase_publishable_key", "api_base_url", "audit.supabase.co", "sb_publishable_closeout_fixture"]) {
  assert.equal(genericConfig.includes(token), false, `generic generated config leaked legacy detail: ${token}`);
}
