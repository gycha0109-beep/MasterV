...
async function main() {
  ...
  for (const bundleSize of requestedSizes) {
    for (let index = 0; index < fixtures.length; index += bundleSize) {
      ...
      try {
        ...
        console.log(`CALIBRATION_PASS size=${bundleSize} bundle=${bundleIndex} duration_ms=${Date.now() - started}`);
      } catch (error) {
        ...
        console.error(`CALIBRATION_FAIL size=${bundleSize} bundle=${bundleIndex} error=${message}`);

        if (isRateLimitError(message)) {
          report.stopped_early = true;
          report.stop_reason = message;
          report.stop_bundle_size = bundleSize;
          report.stop_bundle_index = bundleIndex;
          break calibration;
        }
      }
    }
  ...
  const failedRuns = report.runs.filter((run) => !run.success).length;
  ...
  console.log(`runs=${report.runs.length} success=${report.runs.length - failedRuns} failed=${failedRuns} stopped_early=${report.stopped_early}`);
