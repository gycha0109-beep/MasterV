...
for (const [name, file] of Object.entries(files)) {
  assert.equal(fs.existsSync(file), true, `${name} backend provider asset is missing`);
}
...
const fakeFetch = async () => {
  fetchCalls += 1;
  throw new Error("provider construction must not perform network I/O");
};
...
for (const name of ["boundary", "session", "workData", "remote", "composition"]) {
...
assert.equal(backend.authority.migration_stage, "MV-SUPABASE-EXIT-1B-1");
assert.equal(backend.authority.provider_boundary_active, true);
assert.equal(backend.authority.consumer_wired, false);
...
for (const method of window.MASTERV_BACKEND_PROVIDER_CONTRACT.required_methods.session) {
  assert.equal(typeof backend.session[method], "function", `session provider method missing: ${method}`);
}
for (const method of window.MASTERV_BACKEND_PROVIDER_CONTRACT.required_methods.workData) {
  assert.equal(typeof backend.workData[method], "function", `work-data provider method missing: ${method}`);
}
for (const method of window.MASTERV_BACKEND_PROVIDER_CONTRACT.required_methods.remoteOperations) {
  assert.equal(typeof backend.remoteOperations[method], "function", `remote-operation method missing: ${method}`);
}
