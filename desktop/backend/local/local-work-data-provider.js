(() => {
  ...
  function create(invoke = window.__TAURI__?.core?.invoke) {
    const configured = () => typeof invoke === "function";
    function requireConfigured() {
      if (!configured()) throw new Error("Native local work-data provider is unavailable");
    }
    function assertWorkspace(workspaceId) {
      if (workspaceId && workspaceId !== LOCAL_WORKSPACE_ID) throw new Error(`Local work-data provider rejected non-local workspace: ${workspaceId}`);
    }
    ...
    return Object.freeze({
      ...
      authority: Object.freeze({ workspace: LOCAL_WORKSPACE_ID, persistence: "local-sqlite", product_authority_active: true, server_sync: false })
    });
  }
