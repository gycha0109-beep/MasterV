(() => {
  ...
  function requireCompiler() {
    ...
    if (!compiler || typeof compiler.compile !== "function") throw new Error("Local canonical Reference Compare compiler is unavailable");
    return compiler;
  }

  function create({ gatewaySession, localWorkData, gatewayRemote }) {
    let latestGatewaySource = null;
    const backgroundJobs = new Map();

    const session = Object.freeze({
      configured: () => gatewaySession.configured(),
      ...
      describeSession: (activeSession) => gatewaySession.describeSession(activeSession)
    });

    const workData = Object.freeze({
      configured: () => localWorkData.configured(),
      bootstrapPersonalWorkspace: (activeSession) => localWorkData.bootstrapPersonalWorkspace(activeSession),
      listReferenceLibrary: (activeSession, workspaceId) => localWorkData.listReferenceLibrary(activeSession, workspaceId),
      fetchReferenceDetail: (activeSession, workspaceId, sourceId) => localWorkData.fetchReferenceDetail(activeSession, workspaceId, sourceId),
      deleteReferenceLibraryEntry: (activeSession, workspaceId, sourceId) => localWorkData.deleteReferenceLibraryEntry(activeSession, workspaceId, sourceId),
    ...
    function requireGatewaySession(activeSession) {
      if (activeSession?.provider !== "masterv-gateway") throw new Error("A Product-Key activated MasterV Gateway session is required for paid/remote operations");
    ...
    const snapshotJob = (job) => Object.freeze({ ...job });
    ...
    const remoteOperations = Object.freeze({
      configured: () => gatewayRemote.configured(),
      ...
      discoverYouTube: (activeSession, ...args) => gatewayRemote.discoverYouTube(requireGatewaySession(activeSession), ...args),
      async analyzeYouTube(activeSession, ...args) {
        ...
        return Object.freeze({ ...result, diagnostics: Object.freeze({ ...(result.diagnostics || {}), persistence_writes: 1, persistence_authority: "local-sqlite" }) });
      },
      ...
      async probeBackgroundBatch(activeSession) {
        ...
        return Object.freeze({
          ...
          capabilities: Object.freeze({ boundary_probe: true, local_session_queue: true, gateway_execution: true, local_analysis_persistence: true, desktop_submit_enabled: true, submit: true, restart_durability: false })
        });
      },
      async listBackgroundBatchJobs(activeSession) {
        requireGatewaySession(activeSession);
        return Object.freeze({ contract_version: BACKGROUND_BATCH_CONTRACT_VERSION, jobs: [...backgroundJobs.values()].sort((a, b) => b.created_at.localeCompare(a.created_at)).map(snapshotJob) });
      },
      async submitBackgroundBatchJob(activeSession, requestId, url) {
        ...
        const job = { request_id: id, source_id: null, canonical_url: canonicalUrl, status: "QUEUED", model: null, provider_state: "local-session-queue", error: null, created_at: now, updated_at: now };
