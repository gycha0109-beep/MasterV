(() => {
  ...
  function create(config = {}, fetchImpl = window.fetch.bind(window)) {
    ...
    function requireSession(session) {
      if (!apiBaseUrl || !publishableKey || !apiContractVersion) {
        throw new Error("Legacy hosted API client is not configured");
      }
    ...
    async function probeCapabilities(session) {
      ...
      if (body.contract_version !== apiContractVersion || body.authenticated !== true || body.capabilities?.boundary_probe !== true) {
    ...
    async function compileReferenceWorkflow(session, sourceIds) {
      ...
      if (body.contract_version !== apiContractVersion || body.operation !== "reference_workflow") {
    ...
    async function discoverYouTube(session, query, options) {
      ...
      if (body.contract_version !== apiContractVersion || body.operation !== "youtube_discovery" || body.provider !== "youtube") {
    ...
    async function analyzeYouTube(session, url) {
      ...
      if (body.contract_version !== apiContractVersion || body.operation !== "youtube_deep_analysis" || body.provider !== "gemini") {
    ...
    async function generateProductionGuidance(session, analysis, productTruth) {
      const body = await requestBoundary(session, "POST", {
        operation: "production_guidance",
        analysis,
        product_truth: productTruth
      });
      if (body.contract_version !== apiContractVersion || body.operation !== "production_guidance") {
    ...
    async function requestBackgroundBatch(session, method, payload) {
      ...
      if (!response.ok && !(payload?.operation === "background_batch_check" && response.status === 409)) {
    ...
    async function probeBackgroundBatch(session) {
      ...
      if (body?.contract_version !== "background-batch-hosted-v1" || body?.capabilities?.boundary_probe !== true || body?.capabilities?.durable_ledger !== true) {
        throw new Error("Background Batch capability contract mismatch");
      }
      return body;
    }
    ...
    async function submitBackgroundBatchJob(session, requestId, url) {
      return await requestBackgroundBatch(session, "POST", {
